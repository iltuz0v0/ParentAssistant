...
public class SearchingAdapter extends RecyclerView.Adapter<SearchingViewHolder> implements View.OnClickListener {
    ...
    public SearchingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(searchingAdapterCallback.getAppContext()).inflate(R.layout.searching_element, parent, false);
    ...
    private void lookInPlace(View v) {
        ...
        subMapFragment.setPoints(accounts.get((int) v.getTag()).pointOne, accounts.get((int) v.getTag()).pointTwo);
    ...
    private void lookInAccount(View v) {
        infoWindow.createCompanionRequestWindow(searchingAdapterCallback.getContext(), accounts.get((int) v.getTag()).infoAccounts);
    }
    ...
    private void noInformationToast() {
        ToastManager.showToast(searchingAdapterCallback.getAppContext().getString(R.string.toast_no_elements), searchingAdapterCallback.getAppContext());
    }

    private void fillFullAccountInformation(Account account, OutputAccount outputAccount, int element, List<InfoAccount> infoAccounts) {
    ...
    private void fillInfoAccount(InfoAccount infoAccount, OutputAccount outputAccount, int element) {
    ...
    private void fillAccount(Account account, OutputAccount outputAccount, int element) {
        account.from = outputAccount.getFrom().get(element);
        account.to = outputAccount.getTo().get(element);
        account.pointOne = new LatLng(outputAccount.getPointOneLat().get(element), outputAccount.getPointOneLng().get(element));
        account.pointTwo = new LatLng(outputAccount.getPointTwoLat().get(element), outputAccount.getPointTwoLng().get(element));
    }
